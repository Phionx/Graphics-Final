...
def lit(symbols):
    ...
    for i in symbols:
        ...
        if "ambient" in temp:
            ambient = temp[1:4]
        elif "constants" in temp:
            constants[i] = temp[1]
        elif "light" in temp:
            lights[i] = temp[1]

    info = {"ambient" : ambient, "lights" : lights, "constants" : constants}
    return info
    ...
    for f in range(num_frames):
        ...
        for command in commands:
            ...
            if c == 'set':
                ...
                draw_polygons(tmp, screen, zb, info, args[6])
                tmp = []
            elif c == 'sphere':
                add_sphere(tmp,
                           args[0], args[1], args[2], args[3], step)
                matrix_mult( stack[-1], tmp )
                draw_polygons(tmp, screen, zb, info, args[4])
                tmp = []
            elif c == 'torus':
                add_torus(tmp,
                          args[0], args[1], args[2], args[3], args[4], step)
                matrix_mult( stack[-1], tmp )
                draw_polygons(tmp, screen, zb, info, args[5])
                tmp = []
            elif c == 'move':
                if command[-1]:
